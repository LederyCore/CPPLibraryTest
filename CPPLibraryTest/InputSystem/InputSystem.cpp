...
{
    // #if _USE_RAWINPUTDEVICE 제거 — 항상 등록
...
bool InputSystem::OnHandleMessage(const MSG& msg)
{
    switch (msg.message)
    {
    case WM_INPUT:
...
void InputSystem::HandleMsgKeyUp(WPARAM wParam, LPARAM lParam)
{
    assert(wParam < 256);
...
void InputSystem::HandleKeyboardInput(RAWINPUT& raw)
{
...
void InputSystem::HandleMouseInput(RAWINPUT& raw)
{
    RAWMOUSE& mouse = raw.data.mouse;

    // ── 이동 델타 ─────────────────────────────────────────────
    // Raw Input은 절대좌표가 아닌 프레임 이동량(delta)
    ...
    // ── 버튼 처리 (비트마스크로 한 메시지에 여러 버튼 동시 처리) ──
    auto HandleBtn = [&](USHORT downFlag, USHORT upFlag, bool& pressed, MouseEdge& edge)
        {
            if (mouse.usButtonFlags & downFlag)
            {
                if (!pressed) edge.pressed = true;
                pressed = true;
                edge.released = false;
            }
            if (mouse.usButtonFlags & upFlag)
            {
                if (pressed) edge.released = true;
                pressed = false;
            }
        };

    HandleBtn(RI_MOUSE_LEFT_BUTTON_DOWN, RI_MOUSE_LEFT_BUTTON_UP, m_curMouse.leftPressed, m_mouseEdge[0]);
